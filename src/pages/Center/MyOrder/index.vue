            ...
            <tr>
              <th colspan="5">
                <span class="ordertitle"
                  >{{ order.createTime }}　订单编号：{{ order.outTradeNo }} <span class="pull-right delete"><img src="../images/delete.png" /></span
                ></span>
              </th>
            </tr>
          </thead>
...
<script>
  export default {
    ...
    methods: {
      async getData() {
        const result = await this.$API.reqGetOrder(this.page, this.limit)
        if (result.code === 200) {
